    ...
    public function walletTransactionHistory($wallet_id)
    {
        ...
        if(isset($transactions[0])) {
            foreach ($transactions as $tran) {
                ...
                $tran->deposit_status = deposit_status($tran->status);
            }
...
//        dd($transactions);
    ...
    public function get_wallet_rate($request)
    {
        ...
        try {
            ...
            $data['to_wallet']->coin_type = check_default_coin_type($data['to_wallet']->coin_type);
            $data['from_wallet']->coin_type = check_default_coin_type($data['from_wallet']->coin_type);
            $data['success'] = true;
