        ...
        try {
            ...
            if (isset($members[0])) {
                ...
                foreach ($members as $member) {
                    ...
                    if (isset($find_plan)) {
                        $wallet = get_primary_wallet($member->user_id, DEFAULT_COIN_TYPE);
...
//                            $dollerAmmount = bcmul($bonus,settings()['coin_price'],8);
//                            $btcBonusAmmount = converts_currency($dollerAmmount, $find_plan->bonus_coin_type, $api_rate);
//                        } catch (\Exception $e) {
...
//                        }
                        ...
                        if (empty($bonus_distribution)) {
                            ...
                            $save = $this->saveBonusDistributionHistory($member->user_id,$wallet->id,$member->plan_id,$member->id,date('Y-m-d'),$bonus,$find_plan->bonus,$find_plan->bonus_type,DEFAULT_COIN_TYPE,$bonus);
                        }
                    }
                }
    ...
    public function saveBonusDistributionHistory($user_id,$wallet_id,$plan_id,$membership_id,$distribution_date,$bonus_amount,$plan_current_bonus,$bonus_type,$bonus_coin_type,$bonus)
    {
        try {
            $data = [
              'user_id' => $user_id,
              ...
              'bonus_amount' => $bonus,
              'bonus_amount_btc' => $bonus_amount,
              'plan_current_bonus' => $plan_current_bonus,
              'bonus_type' => $bonus_type,
              'bonus_coin_type' => $bonus_coin_type,
